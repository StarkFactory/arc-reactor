...
- **비용 인식 모델 라우팅** — 쿼리 복잡도와 비용 제약에 따라 최적의 LLM 모델을 자동 선택
- **단계별 토큰 예산** — 단계별 토큰 소비를 소프트/하드 한도로 추적 및 적용
- **Tool 멱등성** — SHA-256 인자 해싱과 TTL 캐시를 통한 중복 Tool 실행 방지
- **실행 체크포인트** — 장애 복구 및 디버깅을 위한 ReAct 루프 중간 상태 저장
- **컨텍스트 인식 Tool 필터링** — 인텐트, 채널, 사용자 역할에 따른 동적 Tool 필터링
- **A2A Agent Card** — A2A 프로토콜에 따라 `/.well-known/agent-card.json`으로 에이전트 기능 노출
...
- [`.env.example`](.env.example) — 로컬 환경 변수 예시
- [`application.yml.example`](application.yml.example) — 애플리케이션 측 속성 예시

MCP 서버 URL, admin token, admin HMAC secret은 `application.yml`에 넣지 않습니다.
`POST /api/mcp/servers`, `PUT /api/mcp/servers/{name}`로 등록하고 갱신합니다.
...
- 핵심 런타임 env만 있으면 됩니다.
- `./gradlew :arc-app:bootRun`으로 시작합니다.

**웹 API + Slack local tool**
...
`send_message`, `reply_to_thread` 같은 Slack LocalTool을 노출할 때 사용하는 모드입니다.
MCP 서버 등록이 아니라 `arc-slack` 모듈의 로컬 도구를 켜는 방식입니다.
...
Grounded answer를 외부 MCP 서버로 라우팅할 때 사용하는 모드입니다.
실제 URL, token, HMAC 값은 관리자 API로 등록한 뒤 access-policy와 preflight를 같은 control plane에서 처리합니다.
검증된 live 흐름은
[docs/ko/operations/mcp-live-runtime-runbook.md](docs/ko/operations/mcp-live-runtime-runbook.md)를 보세요.
...
3. **Channel Plane** — 전송 게이트웨이 (REST, Slack)
...
| 패턴 | 최소 토글/env | 설명 |
|---|---|---|
| 웹 전용 런타임 | 핵심 런타임 env만 | 가장 빠른 로컬 개발 경로 |
| Slack 게이트웨이 | `arc.reactor.slack.enabled=true` + `SLACK_BOT_TOKEN` | Socket Mode면 `SLACK_APP_TOKEN` 추가 |
| Slack local tool | `ARC_REACTOR_SLACK_TOOLS_ENABLED=true` + `SLACK_BOT_TOKEN` | MCP 없이 로컬 Slack 도구 활성화 |
| 외부 MCP 서버 | `ARC_REACTOR_MCP_ALLOWED_SERVER_NAMES=...` | 실제 URL/token/HMAC는 `/api/mcp/servers`로 등록 |
| MCP admin HMAC | `config.adminHmacRequired=true` + `config.adminHmacSecret` | 업스트림 admin API가 HMAC를 요구할 때만 사용 |
...
| 모델 라우팅 | OFF | `arc.reactor.model-routing.enabled` |
| 토큰 예산 | OFF | `arc.reactor.budget.enabled` |
| Tool 멱등성 | OFF | `arc.reactor.tool-idempotency.enabled` |
| 체크포인트 | OFF | `arc.reactor.checkpoint.enabled` |
| Tool 필터링 | OFF | `arc.reactor.tool-filter.enabled` |
...
사용 가능한 태그: 정확한 버전(예: `5.12.0`), 마이너 스트림(예: `5.12`), 짧은 SHA(`sha-<commit>`).
...
| MCP preflight 프록시 | `/api/mcp/servers/{name}/preflight` | 항상 |
| MCP Swagger catalog 프록시 | `/api/mcp/servers/{name}/swagger/sources` | 항상 |
...
- `/api/mcp/servers*`, `/api/tool-policy` 등 control plane 쓰기 API는 admin JWT가 필요합니다.
- chat API는 JWT와 tenant context가 필요합니다.
- MCP 서버는 REST API로만 등록합니다. 서버 URL을 `application.yml`에 직접 넣지 마세요.
...
### 예시: access-policy 프록시용 Atlassian MCP 등록

Atlassian MCP access-policy 프록시를 쓰려면 `config`에 admin 인증값을 포함합니다.
...
그 다음 Arc Reactor를 통해 정책을 프록시합니다.
...
운영 배포 기준이라면 `ok=true`, `readyForProduction=true`가 반환되어야 합니다.
...
- [관리자 API 레퍼런스](docs/ko/admin/api-reference.md)
...
- [MCP Live 연동 런타임 런북](docs/ko/operations/mcp-live-runtime-runbook.md)
