...
| File | Role |
|------|------|
| `agent/impl/SpringAiAgentExecutor.kt` | 핵심 ReAct 루프 (~687 lines). 수정 시 극도로 주의 |
...
| `agent/config/AgentPolicyAndFeatureProperties.kt` | 기능 토글 기본값 (~358 lines, 분할 후) |
...
| **메시지 쌍 무결성** | 대화 히스토리 트리밍 시 AssistantMessage+ToolResponseMessage 쌍 보존 | `hardening/MessagePairIntegrityHardeningTest.kt` |
...
| `model-routing.enabled` | false | `budget.max-tokens-per-request` | 50000 |
| `output-guard.enabled` | true | `tool-result-cache.enabled` | true |
