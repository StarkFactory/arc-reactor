...
각 반복에서 **렌즈 1개만** 사용. 여러 렌즈를 한 반복에 합치지 않는다.
...
2. 타겟 테스트 (수정 모듈 기준):
   ...
   - 리프: arc-web, arc-admin, arc-slack
...
**커밋 정책**:
- **코드 변경(fix/test)만 개별 커밋** — 수정/테스트 추가 시 즉시 커밋
- **docs 커밋은 수렴 시 1회만** — 렌즈별 탐색 결과는 파일에만 기록하고, 수렴 판정 직전에 IMPLEMENTATION_PLAN.md + KNOWN_ACCEPTABLE.md + claude-progress.txt를 한 번에 커밋
- 의미없는 중간 docs 커밋 금지 (렌즈 탐색 결과 기록, 수렴 검증 기록 등)

**기록**: IMPLEMENTATION_PLAN.md `- [x]` + 커밋 해시. claude-progress.txt에 1줄 append. (커밋은 위 정책 따름)

---

## 완료 조건

**매 반복은 반드시 1단계(탐색)부터 시작한다.** 이전 세션 결과를 읽는 것만으로 수렴 판정 불가 — 이번 세션에서 직접 스캔을 수행해야 한다.

P0~P3 미완료 0개 + **이번 세션에서 수행한** 탐색 P0~P3 신규 0건 → `git push origin dev` → `<promise>IMPROVEMENTS COMPLETE</promise>`

**수렴**: 8렌즈 전부 1회 이상 + 마지막 전체 사이클 P0~P3 신규 0건, 또는 20회 초과 시 P0~P1만.
**종료 전 필수**: `git push origin dev`로 모든 커밋을 리모트에 반영한 후 promise 출력.
