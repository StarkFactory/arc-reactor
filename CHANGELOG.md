...
### Added
- GitHub Release automation workflow on tag push (`v*`) via `.github/workflows/release.yml`.
- CI test duration guard script (`scripts/ci/run-with-duration-guard.sh`) and workflow budgets.

### Changed
- Refactored MCP manager into focused components:
  - `McpConnectionSupport`
  - `McpReconnectionCoordinator`
  - `McpStoreSync`
- CI now enforces duration budgets for default and integration test suites.
- CI integration gate now verifies external MCP integration tests are excluded.

### Fixed
- Preserved coroutine cancellation semantics in MCP tool callback error handling.
