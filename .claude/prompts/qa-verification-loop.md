...
9. (선택) 현재 라운드 주제가 §0.4 "참조가 유용할 영역"과 겹치면 해당 레포 문서 경량 스캔
...
[ ] 22. quality gate 판정 기록
[ ] 23. self-development면 loop_health_delta 기록
[ ] 24. 실패 게이트 해당 여부 확인 (해당 시 stop, push 금지)
[ ] 25. `docs/reports/rounds/R{N}.md` 상세 보고서를 고정 스키마대로 작성
[ ] 26. 메인 상태판의 "10. 반복 검증 이력"에 최근 Round 요약 추가
[ ] 27. 최근 20개 요약만 유지하는지 확인
[ ] 28. git add + commit + push
[ ] 29. TaskList 완료 마킹(없으면 최근 Round 요약 + 상세 보고서에 상태 기록), 다음 Round 후보 정리
```
