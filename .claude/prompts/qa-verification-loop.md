...
8. (선택) 현재 라운드 주제가 §0.4 "참조가 유용할 영역"과 겹치면 해당 레포 문서 경량 스캔
...
[ ] 4. benchmark axis 선택
[ ] 5. baseline metric / 기존 gap 확인
[ ] 6. Atlassian eval set에서 관련 case 2개 이상 선택
[ ] 7. runtime backlog에서 이번 Round backlog item 1개 선택
[ ] 8. 최근 5개 Round와 비교해 같은 작업 반복인지 확인
[ ] 9. TaskList(없으면 §2 우선순위 표)에서 다음 우선 작업 선택 (in_progress 마킹)
[ ] 10. chosen_axis / current_gap / expected_delta / evaluated_cases / backlog_item / evidence_paths 기록
[ ] 11. direct_value 우선 여부 확인 (불가 시 foundation 이유 기록)
[ ] 12. 대상 파일 전부 Read
[ ] 13. 코드 수정 (opt-in, 한글 주석, 규칙 준수)
[ ] 14. 테스트 작성 (실패 메시지 필수)
[ ] 15. 선택한 eval case 실행
[ ] 16. ./gradlew compileKotlin compileTestKotlin
[ ] 17. ./gradlew :arc-core:test --tests "변경 테스트"
[ ] 18. baseline/after evidence 확보 확인
[ ] 19. quality gate 판정 기록
[ ] 20. 실패 게이트 해당 여부 확인 (해당 시 stop, push 금지)
[ ] 21. `docs/reports/rounds/R{N}.md` 상세 보고서를 고정 스키마대로 작성
[ ] 22. 메인 상태판의 "10. 반복 검증 이력"에 최근 Round 요약 추가
[ ] 23. 최근 20개 요약만 유지하는지 확인
[ ] 24. git add + commit + push
[ ] 25. TaskList 완료 마킹(없으면 최근 Round 요약 + 상세 보고서에 상태 기록), 다음 Round 후보 정리
```
