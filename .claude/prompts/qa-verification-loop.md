...
[ ] 23. quality gate 판정 기록
[ ] 24. self-development면 loop_health_delta 기록
[ ] 25. 실패 게이트 해당 여부 확인 (해당 시 stop, push 금지)
[ ] 26. `docs/reports/rounds/R{N}.md` 상세 보고서를 고정 스키마대로 작성
[ ] 27. 메인 상태판의 "10. 반복 검증 이력"에 최근 Round 요약 추가
[ ] 28. 최근 20개 요약만 유지하는지 확인
[ ] 29. staged 파일에 raw data / `.qa-runtime/` / `.qa-live/`가 없는지 확인
[ ] 30. git add + commit + push
[ ] 31. TaskList 완료 마킹(없으면 최근 Round 요약 + 상세 보고서에 상태 기록), 다음 Round 후보 정리
```
