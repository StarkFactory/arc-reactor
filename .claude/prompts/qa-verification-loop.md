...
3. `TaskList` 확인 — pending/in_progress Directive 태스크 파악
   ...
   - 이 경우 상태 관리는 별도 파일이 아니라 `docs/production-readiness-report.md`의 최근 Round 기록으로 대신한다.
4. 최근 5개 Round의 `axis / delta / remaining gap / foundation/direct_value` 확인
5. (선택) 현재 라운드 주제가 §0.4 "참조가 유용할 영역"과 겹치면 해당 레포 문서 경량 스캔
...
별도 TaskList가 없다면, 이 상태 변화는 `docs/production-readiness-report.md`의 최신 Round에서
"현재 진행 중 / 이번 Round 완료" 문장으로 기록한다.
...
[ ] 6. TaskList(없으면 §2 우선순위 표)에서 다음 우선 작업 선택 (in_progress 마킹)
[ ] 7. chosen_axis / current_gap / expected_delta / evidence_paths 기록
[ ] 8. direct_value 우선 여부 확인 (불가 시 foundation 이유 기록)
[ ] 9. 대상 파일 전부 Read
[ ] 10. 코드 수정 (opt-in, 한글 주석, 규칙 준수)
[ ] 11. 테스트 작성 (실패 메시지 필수)
[ ] 12. ./gradlew compileKotlin compileTestKotlin
[ ] 13. ./gradlew :arc-core:test --tests "변경 테스트"
[ ] 14. baseline/after evidence 확보 확인
[ ] 15. 보고서 Round N 섹션 추가 (Benchmark Axis / Evidence / Remaining Gap / Run Safety Decision 포함)
[ ] 16. git add + commit + push
[ ] 17. TaskList 완료 마킹(없으면 최근 Round에 상태 기록), 다음 Round 후보 정리
```
