...
| 코드 변경 없음 + 미해결 `[ ]` **1건 이상** | **기존 항목 재검증** | 기준선 9건 + 탐색 5~8건 + **강화 테스트 필수 1~2건** |
...
**기존 항목 재검증 시 필수 행동:**
- 기준선 9건 전부 실행
- 탐색 테스트 5~8건 (이전 감사와 다른 시나리오)
- **강화 테스트 최소 1건** (새 공격 벡터)
- 코드 분석 에이전트 활용 가능 (성능, 보안, 품질)
...
- 코드 수정(fix/refactor)이 있을 때만 AUDIT_CHECKLIST.md 업데이트 + 커밋
...
7. **하트비트 남용 금지** — 미해결 건수 ≥1이면 하트비트 불가. 반드시 탐색+강화 실행
8. **불필요 커밋 금지** — 문제 없으면 커밋하지 않음. 새 발견/해결 시에만 커밋
