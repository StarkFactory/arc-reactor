...
### 13.1 실행 주기
- **자동 실행**: cron `3,13,23,33,43,53 * * * *`로 등록되어 **10분 주기**로 iteration 1회씩 자동 실행
- 1 iteration = 1 페이지 완성 (또는 §16에 따라 1 하위 페이지)
- 10분 안에 페이지가 완성되지 않으면 `IN_PROGRESS`로 유지, 다음 iteration이 이어서 재개
...
- **종료 시 필수 동작 (hard rule)**:
  1. `CronList`로 본 루프 cron ID 조회
  2. `CronDelete`로 해당 cron 취소 (다음 fire 차단)
  3. 최종 완료 보고서 출력: 총 페이지 수 / Phase별 집계 / 누적 claims 통계 / VERIFICATION_FAILED 목록
  4. 사용자에게 "루프 종료" 명시적으로 알림
- 매 iteration 시작 시 §3 진행률을 확인하여 전부 DONE이면 **새 페이지를 작성하지 말고** 위 종료 시퀀스를 실행
