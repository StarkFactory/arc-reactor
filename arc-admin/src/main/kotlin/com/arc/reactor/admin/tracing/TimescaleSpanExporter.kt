...
class TimescaleSpanExporter(
    ...
    private fun batchInsertSpans(spanList: List<SpanData>) {
    ...
    private fun setSpanValues(ps: PreparedStatement, span: SpanData) {
        ...
        ps.setString(2, span.attributes.get(TENANT_ID_KEY) ?: "default")
        ...
        ps.setString(6, span.attributes.get(RUN_ID_KEY))
        ps.setString(7, span.name)
        ps.setString(8, span.resource.attributes.get(SERVICE_NAME_KEY) ?: "arc-reactor")
        ps.setLong(9, durationMs)
        ps.setBoolean(10, success)
        ps.setString(11, if (!success) span.attributes.get(ERROR_TYPE_KEY) else null)
        ps.setString(12, spanAttributesToJson(span))
    }
    ...
    private fun spanAttributesToJson(span: SpanData): String {
        ...
        return map.entries.joinToString(",", "{", "}") { (k, v) ->
            "\"${JsonEscaper.escape(k)}\":\"${JsonEscaper.escape(v)}\""
        }
    }
