...
2. `git log --oneline -10` — 최근 커밋 확인
3. `IMPLEMENTATION_PLAN.md` 읽기 — 미완료 항목 중 최상위 우선순위 1개 선택

## 역할

너는 `IMPLEMENTATION_PLAN.md`의 항목을 하나씩 구현하는 에이전트다.
**반복당 1개 항목만 처리한다.**

## 구현 순서

### 1. 항목 선택
`IMPLEMENTATION_PLAN.md`에서 P0 → P1 → P2 → P3 → P4 순으로 미완료(`- [ ]`) 항목 중 첫 번째를 선택한다.

### 2. 코드 탐색 (구현 전 필수)
**절대 "없을 것"이라고 가정하지 않는다. 반드시 먼저 검색한다.**
```bash
# 관련 파일, 클래스, 함수 먼저 검색
grep -rn "<키워드>" --include="*.kt"
```
...
```bash
git checkout dev
git checkout -b improve/<짧은-설명>   # 예: improve/fix-cancellation-exception-guard
```
...
검증 실패 시:
- 원인 파악 → 수정 → 재검증
- 3회 시도 후도 실패하면: 변경사항 `git checkout .`으로 되돌리고 다른 항목 선택
...
`IMPLEMENTATION_PLAN.md`의 P0~P2 항목이 모두 `- [x]` 이면:
