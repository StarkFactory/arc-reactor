...
internal class SystemPromptBuilder(
    ...
    private fun StringBuilder.appendDuplicateToolCallPreventionHint() {
        ...
        append("\n[Ambiguous Name Disambiguation — 모호한 이름 처리]\n")
        append("Project/repo 이름이 모호할 때(예: 'BB30 저장소', 'JAR 프로젝트', 'web-labs'):\n")
        append("- 사용자가 '저장소'라고 언급했는데 이름이 Jira 프로젝트 키처럼 보이면 ")
        append("(`[A-Z]+-?\\d+` 또는 전부 대문자 + 숫자) → ")
        append("먼저 `bitbucket_list_repositories`로 사용 가능한 레포를 확인하고, ")
        append("일치하는 레포가 없으면 Jira 프로젝트로 해석하여 `jira_search_issues` 또는 ")
        append("`jira_list_projects`로 처리하세요.\n")
        append("- 사용자가 '프로젝트'라고 언급했는데 이름이 레포 slug처럼 보이면 ")
        append("(소문자, 하이픈 포함) → ")
        append("`jira_list_projects`로 Jira 프로젝트 존재 여부부터 확인.\n")
        append("- 절대 도구 호출 없이 '해당 이름을 찾을 수 없습니다'라고 포기하지 마세요. ")
        append("반드시 list 도구로 한 번 탐색한 뒤 결과를 보여주세요.\n")
    }
    ...
    private fun StringBuilder.appendResponseQualityInstruction() {
        ...
        append("[도구 응답의 insights 필드 활용 — 매우 중요]\n")
        append("도구 응답 JSON에 `insights` 필드가 있으면 **반드시 응답의 3번 \"인사이트\" 섹션에 포함**하세요.\n")
        append("insights 필드는 서버에서 미리 계산된 수치·추세·이상치입니다. 직접 다시 세지 말고 그대로 활용하세요.\n")
        append("예시 (bitbucket_list_prs 응답):\n")
        append("  \"insights\": [\"총 12건\", \"24시간+ 미업데이트: 5건\", \"리뷰어 미지정: 2건\", \"평균 수명: 4.3일\"]\n")
        append("→ 응답: '열린 PR 총 12건입니다. 💡 24시간 이상 업데이트 없는 PR 5건, ")
        append("리뷰어가 없는 PR 2건이 있어 정리가 필요해 보입니다. 평균 PR 수명은 4.3일입니다.'\n")
        append("insights가 비어있으면 (빈 배열) 수치 인사이트 섹션은 생략해도 좋습니다.\n\n")
