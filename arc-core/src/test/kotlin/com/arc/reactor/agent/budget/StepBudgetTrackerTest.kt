...
class StepBudgetTrackerTest {
    ...
    fun `토큰을 기록하면 누적되어야 한다`() {
        ...
        tracker.trackStep("step-1", inputTokens = 100, outputTokens = 50)
        tracker.trackStep("step-2", inputTokens = 200, outputTokens = 100)
    ...
    fun `소프트 리밋 미만이면 OK를 반환해야 한다`() {
        ...
        val status = tracker.trackStep("llm-call", inputTokens = 300, outputTokens = 100)
    ...
    fun `소프트 리밋에 도달하면 SOFT_LIMIT을 반환해야 한다`() {
        ...
        // 800 토큰 = 80% → 소프트 리밋
        val status = tracker.trackStep("llm-call", inputTokens = 500, outputTokens = 300)
    ...
    fun `소프트 리밋 초과 후 하드 리밋 미만이면 SOFT_LIMIT을 유지해야 한다`() {
        ...
        tracker.trackStep("step-1", inputTokens = 400, outputTokens = 200) // 600 → OK
        val status = tracker.trackStep("step-2", inputTokens = 200, outputTokens = 100) // 900 → SOFT_LIMIT
    ...
    fun `하드 리밋에 도달하면 EXHAUSTED를 반환해야 한다`() {
        ...
        val status = tracker.trackStep("llm-call", inputTokens = 600, outputTokens = 400)
    ...
    fun `초과 소비해도 remaining은 0 이상이어야 한다`() {
        ...
        tracker.trackStep("overflow", inputTokens = 500, outputTokens = 500)
    ...
    fun `히스토리에 모든 단계가 기록되어야 한다`() {
        ...
        tracker.trackStep("llm-1", inputTokens = 100, outputTokens = 50)
        tracker.trackStep("tool-search", inputTokens = 0, outputTokens = 200)
        tracker.trackStep("llm-2", inputTokens = 300, outputTokens = 150)
    ...
    fun `음수 토큰은 예외를 던져야 한다`() {
        ...
        assertThrows<IllegalArgumentException>("음수 inputTokens는 예외여야 한다") {
            tracker.trackStep("bad", inputTokens = -1, outputTokens = 0)
        }
        assertThrows<IllegalArgumentException>("음수 outputTokens는 예외여야 한다") {
            tracker.trackStep("bad", inputTokens = 0, outputTokens = -1)
        }
    }
    ...
    fun `소프트 리밋 경계값에서 정확히 SOFT_LIMIT을 반환해야 한다`() {
        ...
        // 정확히 80 토큰 = 80%
        val status = tracker.trackStep("exact", inputTokens = 80, outputTokens = 0)
    ...
    fun `하드 리밋 경계값에서 정확히 EXHAUSTED를 반환해야 한다`() {
        ...
        // 정확히 100 토큰 = 100%
        val status = tracker.trackStep("exact", inputTokens = 100, outputTokens = 0)
    ...
    fun `소프트 리밋 바로 한 토큰 미만이면 OK를 반환해야 한다`() {
        ...
        // 79 토큰 = 79% → OK
        val status = tracker.trackStep("just-below", inputTokens = 79, outputTokens = 0)
    ...
    fun `소프트 리밋 바로 한 토큰 초과이면 SOFT_LIMIT을 반환해야 한다`() {
        ...
        // 81 토큰 = 81% → SOFT_LIMIT
        val status = tracker.trackStep("just-above", inputTokens = 81, outputTokens = 0)
    ...
    fun `0 토큰 기록은 상태에 영향을 주지 않아야 한다`() {
        ...
        val status = tracker.trackStep("zero-cost-tool", inputTokens = 0, outputTokens = 0)
    ...
    fun `여러 번 EXHAUSTED 이후에도 상태가 EXHAUSTED로 유지되어야 한다`() {
        ...
        tracker.trackStep("step-1", inputTokens = 100, outputTokens = 0) // EXHAUSTED
        val status = tracker.trackStep("step-2", inputTokens = 10, outputTokens = 0) // 여전히 EXHAUSTED
    ...
    fun `단계 이름이 빈 문자열이어도 기록되어야 한다`() {
        ...
        val status = tracker.trackStep("", inputTokens = 100, outputTokens = 50)
    ...
    fun `히스토리의 StepRecord에 상태가 정확히 기록되어야 한다`() {
        ...
        tracker.trackStep("ok-step", inputTokens = 50, outputTokens = 20)       // 70 → OK
        tracker.trackStep("soft-step", inputTokens = 10, outputTokens = 5)      // 85 → SOFT_LIMIT
        tracker.trackStep("exhausted-step", inputTokens = 10, outputTokens = 10) // 105 → EXHAUSTED
    ...
    fun `softLimitPercent=1은 허용되어야 한다`() {
        ...
        val statusBelow = trackerBelow.trackStep("below", inputTokens = 9, outputTokens = 0)
        val statusAt = trackerAt.trackStep("at", inputTokens = 10, outputTokens = 0)
    ...
    fun `softLimitPercent=99는 허용되어야 한다`() {
        ...
        val status = tracker.trackStep("step", inputTokens = 98, outputTokens = 0)
    ...
    @Test
    fun `recordToolOutput과 trackStep이 함께 누적되어야 한다`() {
        val tracker = StepBudgetTracker(maxTokens = 1000, softLimitPercent = 80)

        tracker.trackStep("llm-call-1", inputTokens = 200, outputTokens = 100) // 300
        tracker.recordToolOutput("tool-output-search", toolOutputTokens = 400) // 700
        val status = tracker.trackStep("llm-call-2", inputTokens = 200, outputTokens = 50) // 950
