...
arc:
  reactor:
    cache:
      ...
      semantic:
        enabled: false            # Redis 시맨틱 캐시 (기본값: false)
...
| `semantic.enabled` | `false` | Redis 기반 시맨틱 캐시 폴백 활성화 |
| `semantic.similarity-threshold` | `0.92` | 시맨틱 히트 최소 코사인 유사도 |
| `semantic.max-candidates` | `50` | 시맨틱 조회 시 평가할 후보 수 |
| `semantic.max-entries-per-scope` | `1000` | 스코프별 Redis 시맨틱 캐시 최대 보관 수 |
...
| `RedisSemanticResponseCache` | Redis 기반 exact+semantic 캐시 (`cache.semantic.enabled=true` + Redis + EmbeddingModel 필요) |
