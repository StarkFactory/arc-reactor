...
`arc.reactor.slack.tools.canvas.enabled=true`이면 2개의 추가 canvas 도구가 등록됩니다:
- `create_canvas` — 봇 토큰 소유의 새 Slack canvas 생성
- `append_canvas` — 기존 Slack canvas에 markdown 콘텐츠 추가
...
| `SlackResponseTextFormatter` | 에이전트 결과를 Slack용으로 포맷하며, 일반 거부 감지 및 폴백 처리 |
| `SlackSlashIntentParser` | 슬래시 명령어 텍스트를 타입화된 의도로 파싱 (agent, brief, my-work, remind, help) |
| `SlackReminderStore` | 시간 파싱(영어 및 한국어)을 지원하는 인메모리 사용자별 리마인더 저장소 |
| `SlackReminderScheduler` | 예정된 리마인더를 폴링하여 Slack API DM으로 알림 전달 |
| `SlackUserEmailResolver` | 인메모리 캐시를 사용하여 `users.info`로 Slack 사용자 이메일 해석 |
| `SlackBotResponseTracker` | 이모지 반응 피드백 상관관계를 위한 봇 응답 메시지 추적 |
| `SlackThreadTracker` | 후속 이벤트 처리 범위 지정을 위해 Arc Reactor가 시작한 스레드 추적 |
| `ProactiveChannelStore` | 프로액티브 모니터링 채널 설정 관리 인터페이스 |
...
| `user-email-resolution-enabled` | `true` | Slack `users.info`에서 요청자 이메일 해석 |
| `user-email-cache-ttl-seconds` | `3600` | 해석된 Slack 사용자 이메일의 인메모리 캐시 TTL |
| `user-email-cache-max-entries` | `20000` | 사용자 이메일 캐시 최대 항목 수 |
| `thread-tracking-enabled` | `true` | Arc Reactor가 시작한 스레드 추적 |
| `thread-tracking-ttl-seconds` | `86400` | 추적된 스레드 보존 시간 |
| `thread-tracking-max-entries` | `20000` | 추적 스레드 최대 항목 수 |
| `process-direct-messages-without-thread` | `false` | `thread_ts` 없이도 최상위 DM 처리 |
| `proactive-enabled` | `false` | 프로액티브 채널 모니터링 활성화 |
| `proactive-channel-ids` | `[]` | 프로액티브 모니터링 채널 ID |
| `proactive-max-concurrent` | `2` | 최대 동시 프로액티브 평가 수 |
| `reaction-feedback-enabled` | `true` | 봇 응답의 이모지 반응에서 피드백 수집 |
| `user-memory-enabled` | `true` | 에이전트 시스템 프롬프트에 사용자별 장기 메모리 주입 |
...
`SlackSocketModeGateway`는 `SmartLifecycle`을 구현하며 애플리케이션 컨텍스트가 준비된 후 시작됩니다. 실패 시 지수 백오프로 연결하고, 연결 해제 시 자동 재연결하며, Events API 봉투, 슬래시 명령어 봉투, 대화형 페이로드를 위한 리스너를 등록합니다. 대화형 페이로드는 로그만 남기고 처리하지 않습니다 (미지원).
