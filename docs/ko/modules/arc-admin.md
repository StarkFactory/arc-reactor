...
| `AdminAuthHelper` | 관리자 컨트롤러용 공유 `isAdmin()`, `isAnyAdmin()`, `forbiddenResponse()` 헬퍼 |
...
쓰기 엔드포인트는 `AdminAuthHelper.isAdmin(exchange)`(개발자 관리자 스코프)를 사용하고, 대시보드/조회 전용 엔드포인트는 `AdminAuthHelper.isAnyAdmin(exchange)`(매니저 관리자 포함)를 사용할 수 있습니다. 역할 정보가 없으면 기본적으로 비관리자로 차단됩니다.
