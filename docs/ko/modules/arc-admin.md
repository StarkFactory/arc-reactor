...
| `MetricIngestionController` | `POST /api/admin/metrics/ingest` — MCP 서버 및 외부 소스의 메트릭 수집 |
| `AdminAuthSupport` | 관리자 컨트롤러용 공유 `isAdmin()`, `isAnyAdmin()`, `forbiddenResponse()` 헬퍼 |
| `AdminAuditSupport` | 감사 로그 영속성을 위한 공유 `recordAdminAudit()` 헬퍼 |
| `MetricGuardAuditPublisher` | Guard 감사 이벤트를 링 버퍼에 발행 (SOC 2 준수를 위한 SHA-256 해싱된 입력) |
| `PipelineHealthMonitor` | 파이프라인 헬스 메타 메트릭 추적 (쓰기 지연, 에러, 드롭, 캐시 히트) |
| `ExportService` | 테넌트 실행 및 도구 호출 데이터의 JDBC 기반 CSV 내보내기 |
| `BaselineCalculator` | 이상 탐지 알림을 위한 캐시된 평균/표준편차 베이스라인 계산 |
| `AlertNotificationService` | 설정된 채널(로그, Slack, 웹훅)을 통한 알림 전송 |
| `TenantSpanProcessor` | 모든 트레이스 span에 `tenant_id`를 주입하는 OTel `SpanProcessor` |
| `OtlpExporterConfiguration` | OTLP span 내보내기(gRPC 또는 HTTP) 및 TimescaleDB 내보내기 구성 |
...
| `privacy.store-user-identifiers` | `false` | 관리자 메트릭/트레이스에 `userId` 저장 여부 |
| `privacy.store-session-identifiers` | `false` | 관리자 메트릭/트레이스에 `sessionId` 저장 여부 |
...
쓰기 엔드포인트는 `AdminAuthSupport.isAdmin(exchange)`(개발자 관리자 스코프)를 사용하고, 대시보드/조회 전용 엔드포인트는 `AdminAuthSupport.isAnyAdmin(exchange)`(매니저 관리자 포함)를 사용할 수 있습니다. 역할 정보가 없으면 기본적으로 비관리자로 차단됩니다.
...
**OTLP와 TimescaleDB 추적은 독립적입니다.** 둘 다 동시에 활성화할 수 있습니다. OTLP는 기본 비활성이며, `tracing.otlp.enabled=true`로 설정하고 엔드포인트를 제공하면 활성화됩니다.
...
**추적에서의 HITL 대기 감지.** `AgentTracingHooks`는 전체 경과 시간과 도구 실행 시간을 비교하여 HITL 대기 시간을 추정합니다. 차이가 100ms를 초과하면 span에 `gen_ai.tool.hitl.required=true` 태그를 추가합니다.
