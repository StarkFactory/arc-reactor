...
| P0-2. Chat Guard/Filtering | `/api/chat` (oversized + tenant mismatch) | `VUS=5`, `1m` | 계약 검증 실패율 < 1%, 예상 외 상태 < 1% |
| P0-3. Chat Stream Security | `/api/chat/stream` (oversized + tenant mismatch) | `VUS=5`, `1m` | 계약 검증 실패율 < 1%, 예상 외 상태 < 1% |
| P0-4. Chat Stream Soak | `/api/chat/stream` (장시간) | `VUS=10`, `20m` | 계약 검증 실패율 < 1%, 예상 외 상태 < 1%, 장시간 안정성 확보 |
...
# Chat Guard/Filtering baseline (no LLM dependency)
