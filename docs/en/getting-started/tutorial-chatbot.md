...
- **Add more tools**: Connect to databases, APIs, or external services. See the [Tool Guide](../reference/tools.md)
- **Enable authentication**: Add JWT auth for per-user sessions. See [Authentication](../governance/authentication.md)
- **Enable RAG**: Add document-based Q&A with vector search. See [Memory & RAG](../architecture/memory-rag.md)
- **Connect MCP servers**: Add external tools via Model Context Protocol. See [MCP Guide](../architecture/mcp.md)
- **Add Guard stages**: Implement custom content filtering or permission checks. See [Guard & Hook](../architecture/guard-hook.md)
- **Multi-agent patterns**: Build a supervisor that delegates to specialized worker agents. See [Multi-Agent Guide](../architecture/multi-agent.md)
